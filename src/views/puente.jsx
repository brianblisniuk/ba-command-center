...
(function () {
  ...
  function Copiloto({ toast, nav, openTrip }) {
    ...
    function chipGo(c) {
      ...
      const firstTrip = window.BA.salidas.find(s => ['risk', 'go'].includes(s.estado)) || window.BA.salidas[0];
      ...
      else if (/acceso|salida|uzbek|itinerario/.test(t)) firstTrip ? openTrip(firstTrip.id) : nav('viajes');
      else nav('bandeja');
      toast(c);
    }
  ...
  function WorldMap({ openTrip, cur }) {
    ...
    const o = px(origin);
    const sal = window.BA.salidas.filter(s => Number.isFinite(s.lat) && Number.isFinite(s.lng));
    function arc(a, b) {
