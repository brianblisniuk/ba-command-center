...
(function () {
  ...
  // ============ ITINERARIO ============
  function Itinerario({ s, cur, toast }) {
    ...
    return React.createElement('div', null,
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16, flexWrap: 'wrap' } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: 8 } },
          React.createElement('button', { className: 'btn sm', onClick: () => setOpen(allOpen ? new Set() : new Set(data.map(d => d.n))) }, React.createElement(Icon, { name: 'layers' }), allOpen ? 'Colapsar todo' : 'Expandir todo'),
          React.createElement('button', { className: 'btn sm primary', onClick: () => toast('Día agregado') }, React.createElement(Icon, { name: 'plus' }), 'Agregar día'))
      ),
      capa === 'cliente' && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 9, padding: '11px 15px', background: 'var(--curso-bg)', borderRadius: 'var(--radius-sm)', marginBottom: 16, fontSize: 12.5, color: 'var(--text-2)' } },
        React.createElement(Icon, { name: 'eye', style: { width: 16, height: 16, color: 'var(--curso)' } }), 'Capa narrativa — así lo ve el huésped: sin precios, sin notas operativas, sin accesos en gestión.'),
      React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
        data.map(day => {
          const isOpen = open.has(day.n);
          const slots = capa === 'cliente' ? day.slots.filter(sl => !(sl.access && sl.verdict === 'En gestión')) : day.slots;
          return React.createElement('div', { key: day.n, className: 'card', style: { overflow: 'hidden' } },
            React.createElement('div', { onClick: () => toggle(day.n), style: { display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', cursor: 'pointer', borderLeft: '4px solid ' + day.cover } },
              React.createElement('div', { style: { width: 46, textAlign: 'center', flexShrink: 0 } },
                React.createElement('div', { className: 'figure', style: { fontSize: 24, color: 'var(--text-1)' } }, day.n),
                React.createElement('div', { className: 'eyebrow' }, day.dow)),
              React.createElement('div', { style: { flex: 1, minWidth: 0 } },
                React.createElement('div', { style: { fontSize: 15, fontWeight: 650, color: 'var(--text-1)' } }, day.title),
                React.createElement('div', { style: { fontSize: 11.5, color: 'var(--text-3)', marginTop: 2, fontFamily: 'var(--ff-mono)' } },
            ...
            isOpen && React.createElement('div', { style: { padding: '4px 18px 18px 18px' } },
              slots.map((sl, i) => {
                const st = BA.STYPE[sl.type];
                return React.createElement('div', { key: i, style: { display: 'flex', gap: 14, padding: '12px 0', borderTop: i ? '1px solid var(--rule-soft)' : 'none' } },
                  React.createElement('div', { style: { width: 86, flexShrink: 0 } },
                    React.createElement('div', { className: 'mono', style: { fontSize: 12, fontWeight: 600, color: 'var(--text-1)' } }, sl.time),
                    React.createElement('div', { className: 'mono', style: { fontSize: 10.5, color: 'var(--text-faint)' } }, sl.end)),
                  React.createElement('div', { style: { width: 4, borderRadius: 99, background: st.c, flexShrink: 0 } }),
                  React.createElement('div', { style: { flex: 1, minWidth: 0 } },
                    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
                      React.createElement('span', { style: { fontSize: 14, fontWeight: 600, color: 'var(--text-1)' } }, sl.title),
                      sl.access && React.createElement('span', { className: 'badge risk', style: { padding: '2px 7px' } }, React.createElement(Icon, { name: 'key', style: { width: 10, height: 10 } }), 'Acceso'),
                    ...
                    React.createElement('div', { style: { fontSize: 12.5, color: 'var(--text-3)', marginTop: 3 } }, sl.desc),
                    capa === 'op' && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 7 } },
                      React.createElement('span', { className: 'tag', style: { padding: '2px 8px' } }, st.t),
                      React.createElement('span', { className: 'badge ' + (sl.verdict === 'Confirmado' ? 'go' : 'risk'), style: { padding: '2px 7px' } }, sl.verdict))),
                  capa === 'op' && React.createElement('button', { className: 'tag', title: sl.client ? 'Visible al cliente' : 'Oculto al cliente', style: { padding: '4px 7px', alignSelf: 'flex-start' }, onClick: () => toast(sl.client ? 'Oculto al cliente' : 'Visible al cliente') },
                    React.createElement(Icon, { name: 'eye', style: { width: 13, height: 13, color: sl.client ? 'var(--accent)' : 'var(--text-faint)' } }))
                );
              })
            )
          );
  ...
  function Ruta({ s, cur, toast }) {
    ...
    return React.createElement('div', null,
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16, flexWrap: 'wrap' } },
        React.createElement('div', { className: 'tb-seg' },
          data.map(d => React.createElement('button', { key: d.n, className: dia === d.n ? 'on' : '', onClick: () => setDia(d.n) }, 'Día ' + d.n))),
      ...
      React.createElement('div', { className: 'grid', style: { gridTemplateColumns: 'minmax(0,1.3fr) minmax(0,1fr)', alignItems: 'start' } },
        // stylized map
        React.createElement('div', { className: 'card pad' },
          React.createElement(CardHead, { title: day.title, right: React.createElement('span', { className: 'eyebrow' }, 'modo día') }),
          React.createElement('div', { style: { position: 'relative', borderRadius: 'var(--radius-sm)', overflow: 'hidden', background: 'var(--surface-2)', aspectRatio: '4/3' } },
            React.createElement('svg', { viewBox: '0 0 100 75', preserveAspectRatio: 'none', style: { position: 'absolute', inset: 0, width: '100%', height: '100%' } },
              [15, 30, 45, 60].map(y => React.createElement('line', { key: y, x1: 0, x2: 100, y1: y, y2: y, stroke: 'var(--rule-soft)', strokeWidth: 0.3 })),
              React.createElement('path', { d: stops.map((sl, i) => { const x = 14 + (i * 72) / Math.max(1, stops.length - 1); const y = 22 + (i % 2 ? 26 : -4) + 18; return (i ? 'L' : 'M') + x + ' ' + y; }).join(' '), fill: 'none', stroke: 'var(--brass)', strokeWidth: 0.7, strokeDasharray: '1.5 1.5' })),
            stops.map((sl, i) => { const x = 14 + (i * 72) / Math.max(1, stops.length - 1); const y = 22 + (i % 2 ? 26 : -4) + 18;
              return React.createElement('div', { key: i, style: { position: 'absolute', left: x + '%', top: y + '%', transform: 'translate(-50%,-50%)', width: 26, height: 26, borderRadius: 9, background: sl.access ? 'var(--brass)' : 'var(--accent)', color: '#fff', display: 'grid', placeItems: 'center', fontFamily: 'var(--ff-mono)', fontSize: 12, fontWeight: 700, boxShadow: 'var(--shadow-sm)', cursor: 'pointer' }, title: sl.title, onClick: () => toast(sl.title) }, i + 1); })
          )
        ),
        // legs
        React.createElement('div', { className: 'card pad' },
          React.createElement(CardHead, { title: 'Tramos del día' }),
          React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 0 } },
            stops.map((sl, i) => React.createElement('div', { key: i },
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 11, padding: '8px 0' } },
                ...
                React.createElement('span', { style: { fontSize: 13, color: 'var(--text-1)', fontWeight: 600 } }, sl.title)),
              legs[i] && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0 2px 35px', fontSize: 11.5, color: 'var(--text-3)' } },
                React.createElement(Icon, { name: legs[i].mode === 'auto' ? 'route' : 'pin', style: { width: 13, height: 13 } }),
                legs[i].mode + ' · ' + legs[i].mins + ' min')
            ))
          )
        )
      )
  ...
  function Proveedores({ s, cur, toast }) {
    ...
    return React.createElement('div', null,
      ...
      React.createElement('div', { className: 'card pad' },
        React.createElement('table', { className: 'tbl' },
          ...
          React.createElement('tbody', null, all.map((p, i) => (tipo === 'all' || p.tipo === tipo) && React.createElement('tr', { key: i },
            ...
            React.createElement('td', null, React.createElement('button', { className: 'badge ' + PEST[estados[i]].c, style: { cursor: 'pointer' }, onClick: () => cycle(i) }, PEST[estados[i]].t)),
            React.createElement('td', { style: { textAlign: 'right', whiteSpace: 'nowrap' } },
              React.createElement('span', { style: { display: 'inline-flex', gap: 4 } },
                ['phone', 'mail', 'pin'].map(ic => React.createElement('button', { key: ic, className: 'tag', style: { padding: '4px 6px' }, onClick: () => toast('Abrir ' + ic) }, React.createElement(Icon, { name: ic, style: { width: 13, height: 13 } })))))
          )))
        )
      )
    );
